...
| `scripts/lib-updates.sh` + `updates-waybar.sh` | **User-visible.** `flatpak_count` discarded `remote-ls`'s exit status, so an unreachable Flathub read as zero updates and the tooltip said "Flatpak: up to date"; the OS `none` branch added nothing to the badge. Grey icon while 11 Flatpak, 1 yazi and 27 OS packages waited. **Fixed 2026-08-18**, test-first. |
