...
- `custom/updates`: calls `~/.local/bin/updates-waybar` every 60s — icon-only `⬆`, always visible (neutral colour when nothing pending). Three CSS severity classes: `uptodate` (neutral, like other icons), `warning` (amber — Flatpak/container updates, no reboot), `critical` (red — Fedora OS update pending OR security advisories present). Hover tooltip lists every section one per line, in menu order: **Flatpak → Containers → OS**. OS shows a **Security / Regular** split (Security = packages with a security advisory, Regular = the rest; they sum to the package total). Containers show per-container last-updated date (from our timestamp, else `podman {{.Created.Unix}}`). Left click opens the update menu. JSON cached 1h; signal 8 forces refresh. **All detection is shared via `scripts/lib-updates.sh`** so the indicator and the menu always agree; containers/apps/OS discovered dynamically (no hardcoded names).
  - **Last-known-good OS cache:** `rpm-ostree upgrade --check` is slow and can fail when the third-party NordVPN rpm repo is briefly unreachable. `os_refresh_cache` retries 3×, persists the last SUCCESSFUL result to `~/.cache/os-check.cache` (+`.ts` timestamp), and when offline falls back to that cached result labelled "(as of DATE, repo offline)" instead of falsely reporting "up to date" — the same pattern apt/dnf use for cached metadata.
