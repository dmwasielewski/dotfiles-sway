...
- `custom/updates`: calls `~/.local/bin/updates-waybar` every 60s — icon-only `⬆`, always visible (neutral colour when nothing pending). Three CSS severity classes: `uptodate` (neutral, like other icons), `warning` (amber — Flatpak/container updates, no reboot), `critical` (red — Fedora OS update pending OR security advisories present). Hover tooltip lists every section one per line, in menu order: **Flatpak → Containers → Language packages → User-local apps → OS**. OS shows a **Security / Regular** split (Security = packages with a security advisory, Regular = the rest; they sum to the package total). Containers show per-container last-updated date (from our timestamp, else `podman {{.Created.Unix}}`). Left click opens the update menu. JSON cached 1h; signal 8 forces refresh. **All detection is shared via `scripts/lib-updates.sh`** so the indicator and the menu always agree; containers/apps/OS discovered dynamically (no hardcoded names).
  ...
  - `updates-menu.sh` (interactive, runs in foot): one OS check per session (shared cache) so summary and list always agree; always shows every section with last-updated dates; menu order by update frequency — 1) Flatpak 2) Containers 3) Language packages 4) User-local apps 5) Fedora OS 6) Everything (continue-on-error with a results summary, no `--force`) 7) Show update list (aligned table `package | current → new`, scrollable via `less` with an on-screen "press q to return" prompt) q) Cancel. OS updates are atomic (rpm-ostree) so a failed upgrade is inherently safe.
